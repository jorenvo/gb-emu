...
export class OpLdA16InRegToA extends Instruction {
  ...
  exec(cpu: CPU, memory: Memory): number {
    ...
    const aHigh = cpu.getReg(register);
    const aLow = cpu.getReg(register + 1);
...
export class OpLdhA8 extends Instruction {
  ...
  exec(cpu: CPU, memory: Memory): number {
    ...
    memory.setByte(0xff00 | lowAddr, cpu.getReg(CPU.A));
    return 12;
  }
...
export class OpLdR8ToR8 extends Instruction {
  ...
  exec(cpu: CPU, memory: Memory): number {
    ...
    if (srcReg === 6) {
      ...
      memory.setByte(cpu.getHL(), cpu.getReg(srcReg));
    } else {
      cpu.setReg(destReg, cpu.getReg(srcReg));
    }
...
export class OpRLA extends Instruction {
  ...
  exec(cpu: CPU, _memory: Memory): number {
    cpu.setReg(CPU.A, cpu.rotateLeft(cpu.getReg(CPU.A)));
    return 4;
  }
...
export class OpRRC extends Instruction {
  ...
  exec(cpu: CPU, memory: Memory): number {
    ...
    // TODO: fix carry, rotateLeft already sets it which is probably wrong
    if (register === 6) {
      cpu.setHL(cpu.rotateRight(cpu.getHL()));
      return 16;
    } else {
      cpu.setReg(register, cpu.rotateRight(cpu.getReg(register)));
      return 8;
    }
  }
...
export class OpAddR16ToHL extends Instruction {
  ...
  exec(cpu: CPU, memory: Memory): number {
    ...
    let hl = cpu.getHL();
    cpu.setHalfCarryFlag16BitAdd(r16, hl);
...
export class OpCallIfCarry extends OpCall {
  exec(cpu: CPU, memory: Memory): number {
    if (cpu.getCarryFlag()) {
      super.exec(cpu, memory);
      return 24;
  ...
  disassemble(memory: Memory) {
    let addr = this.getNext16Bits(memory);
    return `CALL C, $${utils.hexString(addr, 16)}`;
  }
}
...
export class OpLdAToA16 extends Instruction {
  ...
  exec(cpu: CPU, memory: Memory): number {
    const addr = this.getNext16Bits(memory);
    memory.setByte(addr, cpu.getReg(CPU.A));
    return 16;
  }
...
export class OpSubR8 extends Instruction {
  ...
  protected getToSubtract(cpu: CPU, memory: Memory) {
    const reg = this.getReg(memory);
    return this.isHL(memory) ? memory.getByte(cpu.getHL()) : cpu.getReg(reg);
  }
...
export class OpSubCarryD8 extends Instruction {
  size() {
    return 2;
  }
  ...
  exec(cpu: CPU, memory: Memory): number {
    ...
    cpu.setReg(CPU.A, utils.wrapping8BitAdd(cpu.getReg(CPU.A), -d8 - carry));
    cpu.setZeroFlag(cpu.getReg(CPU.A) === 0 ? 1 : 0);
    cpu.setSubtractFlag(1);
...
export class OpAddCarryR8 extends Instruction {
  size() {
    return 1;
  }
  ...
  private isHL(memory: Memory) {
    return this.getByte(memory) === 0x8e;
  }
...
export class OpAddCarryD8 extends Instruction {
  ...
  exec(cpu: CPU, memory: Memory): number {
    ...
    cpu.setHalfCarryFlag8BitAdd(cpu.getReg(CPU.A), d8, carry);
...
abstract class OpBitManip extends Instruction {
  ...
  exec(cpu: CPU, memory: Memory): number {
    ...
    if (this.isHL(memory)) {
      ...
      cpu.setReg(reg, this.manipulateBit(bit, cpu.getReg(reg)));
      return 8;
    }
  }
